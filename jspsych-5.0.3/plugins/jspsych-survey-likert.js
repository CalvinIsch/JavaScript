...
jsPsych.plugins['survey-likert'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // This is the stuff I added to try and make so it all looks good.
    x = true;
    function comon() {
      var isValid = true;
      $("#jspsych-survey-likert-form .jspsych-survey-likert-opts").each(function() {
        var id = $(this).data('radio-group');
        var response2 = $('input[name="' + id + '"]:checked').val();
        if (typeof response2 === 'undefined' ) {
            isValid = false;
            console.log("Hey");
            //setTimeout(comon(), 1000);
        }
      });
      console.log("Worked?");
      if (isValid) { x == false; }
